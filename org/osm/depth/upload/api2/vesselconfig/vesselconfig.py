    ...
    try:
        with connections['osmapi'].cursor() as cursor:
            ...
            if request.user.is_authenticated:
                ...
                cursor.execute(new_vessel_sql, (str(vessel_data['name']), str(vessel_data['description']), "None", vessel_data['manufacturer'], vessel_data['model'], vessel_data['loa'], vessel_data['breadth'], vessel_data['draft'], vessel_data['height'], vessel_data['displacement'], vessel_data['maximumspeed'], vessel_type, str(request.user)))
